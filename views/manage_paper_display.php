...
<div class="wrap">
  <?php
		...
		{
	?>
  			<div class="error-box" style="width:400px;"> 
			<?php
				for($i = 0; $i<count($error_list); $i++)
				{
			?>
				<strong>ERROR</strong>:
				<?php echo $error_list[$i]; ?>
				<br />
			<?php
				}
			?>
  ...
  <?php
	...
	?>
  <h2> <?php echo __( 'White Papers'); ?> </h2>
  <form name="whitepaper_paper_manage_form" 
    			method="post"
    			action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
    <h4> <?php echo __( 'Add New WhitePaper'); ?> </h4>
    <table>
		<tr>
		...
		<tr align="left">
			<td colspan="3">
				<input type="hidden" name="submit_add_white_paper" value="add" />
				<input type="submit" name="submit_addCategory" value="<?php _e('Add Whitepaper') ?>" /></td>
		</tr>
    </table>
  </form>
  <?php
